...
struct Vector<T>::Impl {
  ...
  ::wrp_cte::gpu_vector::VectorStats *stats = nullptr;
...
inline Vector<T>::Vector(const std::string &tag_name, chi::u32 nblocks,
  ...
  if (nblocks == 0 || gpu_pages_per_block == 0 || page_size_bytes == 0) {
    throw std::invalid_argument(
        "wrp_cte::gpu_vector::Vector: nblocks/gpu_pages_per_block/page_size "
        "must all be > 0");
  }
  ...
  if (manager_threads_per_block == 0 ||
    ...
    throw std::invalid_argument(
        "wrp_cte::gpu_vector::Vector: manager_threads_per_block must be a "
  ...
  {
    void *p = nullptr;
    cudaHostAlloc(&p, sizeof(::wrp_cte::gpu_vector::VectorStats),
                  cudaHostAllocMapped | cudaHostAllocPortable);
    impl_->stats = static_cast<::wrp_cte::gpu_vector::VectorStats *>(p);
    if (impl_->stats) {
      std::memset(impl_->stats, 0,
                  sizeof(::wrp_cte::gpu_vector::VectorStats));
    }
...
inline void Vector<T>::DrainHostPrefetchQueue(void *cuda_stream) {
  ...
  std::vector<chi::Future<wrp_cte::core::GetBlobTask>> futs;
