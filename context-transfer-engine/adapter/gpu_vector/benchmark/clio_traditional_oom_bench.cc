...
struct BenchOpts {
  ...
  clio::run::u32 pages_per_block = 2;     // accepted for CLI compat; unused here
...
void EnsureInit(const BenchOpts &opts, clio::run::u64 bdev_capacity_bytes) {
  ...
  auto bdev_create = bdev_client.AsyncCreate(
      clio::run::PoolQuery::Dynamic(), std::string("trad_bench_ram"),
  ...
  auto reg_task = cte_client->AsyncRegisterTarget(
      "trad_bench_ram", clio::run::bdev::BdevType::kRam,
...
__global__ void TradFillPageKernel(clio::run::u32 *dev_buf,
  ...
  clio::run::u32 *slot = dev_buf + b * elems_per_page;
...
__global__ void TradConsumePageKernel(const clio::run::u32 *dev_buf,
                                       clio::run::u64 elems_per_page,
                                       clio::run::u32 *out_xor) {
  clio::run::u32 b = blockIdx.x;
  const clio::run::u32 *slot = dev_buf + b * elems_per_page;
  clio::run::u32 acc = 0;
  for (clio::run::u64 j = threadIdx.x; j < elems_per_page; j += blockDim.x) {
    acc ^= slot[j];
  }
...
int main(int argc, char *argv[]) {
  ...
  clio::run::u32 *dev_buf =
      ctp::GpuApi::Malloc<clio::run::u32>(workspace_bytes);
  clio::run::u32 *host_buf =
      ctp::GpuApi::MallocHost<clio::run::u32>(workspace_bytes);
  if (!dev_buf || !host_buf) {
    std::fprintf(stderr,
  ...
  for (clio::run::u32 it = 0; it < opts.iters; ++it) {
    ...
    for (clio::run::u64 p = 0; p < per_block_pages; ++p) {
      ...
      ctp::GpuApi::Memcpy<clio::run::u32>(host_buf, dev_buf, workspace_bytes);
      for (clio::run::u32 b = 0; b < opts.nblocks; ++b) {
        std::string blob_name = "trad_b" + std::to_string(b) +
                                "_pi" + std::to_string(p);
        ctp::ipc::ShmPtr<> ptr;
        ptr.alloc_id_.SetNull();
        ptr.off_ = reinterpret_cast<clio::run::u64>(host_buf + b * elems_per_page);
    ...
    if (opts.do_read) {
      ...
      for (clio::run::u64 p = 0; p < per_block_pages; ++p) {
        ...
        ctp::GpuApi::Memcpy<clio::run::u32>(dev_buf, host_buf, workspace_bytes);
  ...
  ctp::GpuApi::Free<clio::run::u32>(dev_buf);
  ctp::GpuApi::FreeHost<clio::run::u32>(host_buf);
