...
int main(int argc, char *argv[]) {
  ...
  chi::u32 *dev_buf =
      ctp::GpuApi::Malloc<chi::u32>(workspace_bytes);
  chi::u32 *host_buf =
      ctp::GpuApi::MallocHost<chi::u32>(workspace_bytes);
  if (!dev_buf || !host_buf) {
    std::fprintf(stderr,
  ...
  for (chi::u32 it = 0; it < opts.iters; ++it) {
    ...
    for (chi::u64 p = 0; p < per_block_pages; ++p) {
      ...
      ctp::GpuApi::Memcpy<chi::u32>(host_buf, dev_buf, workspace_bytes);
    ...
    if (opts.do_read) {
      ...
      for (chi::u64 p = 0; p < per_block_pages; ++p) {
        ...
        ctp::GpuApi::Memcpy<chi::u32>(dev_buf, host_buf, workspace_bytes);
  ...
  ctp::GpuApi::Free<chi::u32>(dev_buf);
  ctp::GpuApi::FreeHost<chi::u32>(host_buf);
