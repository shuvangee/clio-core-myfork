...
FILE *CLIO_CTE_DECL(fopen)(const char *path, const char *mode) {
  ...
  if (clio::cte::filesystem::Client::IsPathTracked(path)) {
    HLOG(kDebug, "Intercepting fopen({}, {})", path, mode);
    int fd = CLIO_CFS_CLIENT->OpenFd(path, StdioShim::ModeToFlags(mode), 0644);
    if (fd < 0) {
      return nullptr;
    }
    return Shim().Wrap(fd);
...
FILE *CLIO_CTE_DECL(fdopen)(int fd, const char *mode) {
  auto real_api = CLIO_CTE_STDIO_API;
  if (CLIO_CFS_CLIENT->IsFdTracked(fd)) {
...
FILE *CLIO_CTE_DECL(freopen)(const char *path, const char *mode, FILE *stream) {
  ...
  if (Shim().IsTracked(stream)) {
    ...
    if (old_fd >= 0) {
      CLIO_CFS_CLIENT->CloseFd(old_fd);
    }
    if (!clio::cte::filesystem::Client::IsPathTracked(path)) {
...
int CLIO_CTE_DECL(fflush)(FILE *fp) {
  ...
  if (fd >= 0) {
    ...
    return CLIO_CFS_CLIENT->SyncFd(fd);
  }
...
int CLIO_CTE_DECL(fclose)(FILE *fp) {
  ...
  if (fd >= 0) {
    ...
    return CLIO_CFS_CLIENT->CloseFd(fd);
  }
...
size_t CLIO_CTE_DECL(fwrite)(const void *ptr, size_t size, size_t nmemb,
  ...
  if (fd >= 0) {
    ...
    ssize_t ret = CLIO_CFS_CLIENT->WriteFd(fd, ptr, size * nmemb);
...
int CLIO_CTE_DECL(fputc)(int c, FILE *fp) {
  ...
  if (fd >= 0) {
    ...
    return (CLIO_CFS_CLIENT->WriteFd(fd, &ch, 1) == 1) ? c : EOF;
  }
...
int CLIO_CTE_DECL(fgetpos)(FILE *fp, fpos_t *pos) {
  ...
  if (fd >= 0 && pos) {
    ...
    pos->__pos = CLIO_CFS_CLIENT->TellFd(fd);
    return 0;
  }
...
int CLIO_CTE_DECL(putw)(int w, FILE *fp) {
  ...
  if (fd >= 0) {
    ...
    return (CLIO_CFS_CLIENT->WriteFd(fd, &w, sizeof(w)) ==
...
int CLIO_CTE_DECL(fputs)(const char *s, FILE *stream) {
  ...
  if (fd >= 0) {
    ...
    ssize_t ret = CLIO_CFS_CLIENT->WriteFd(fd, s, std::strlen(s));
...
size_t CLIO_CTE_DECL(fread)(void *ptr, size_t size, size_t nmemb,
  ...
  if (fd >= 0) {
    ...
    ssize_t ret = CLIO_CFS_CLIENT->ReadFd(fd, ptr, size * nmemb);
    return (ret > 0) ? static_cast<size_t>(ret) / size : 0;
  }
...
int CLIO_CTE_DECL(fgetc)(FILE *stream) {
  ...
  if (fd >= 0) {
    ...
    return (CLIO_CFS_CLIENT->ReadFd(fd, &value, 1) == 1) ? value : EOF;
  }
...
int CLIO_CTE_DECL(getw)(FILE *stream) {
  ...
  if (fd >= 0) {
    ...
    int value = 0;
    return (CLIO_CFS_CLIENT->ReadFd(fd, &value, sizeof(int)) ==
...
char *CLIO_CTE_DECL(fgets)(char *s, int size, FILE *stream) {
  ...
  if (fd >= 0) {
    ...
    ssize_t got = CLIO_CFS_CLIENT->ReadFd(fd, s, want);
    if (got <= 0) {
      return nullptr;
    }
    ...
    if (end < static_cast<size_t>(got)) {
      CLIO_CFS_CLIENT->SeekFd(fd, -(static_cast<off_t>(got) - static_cast<off_t>(end)),
                         SEEK_CUR);
    }
    s[end] = '\0';
    return s;
...
void CLIO_CTE_DECL(rewind)(FILE *stream) {
  ...
  if (fd >= 0) {
    ...
    CLIO_CFS_CLIENT->SeekFd(fd, 0, SEEK_SET);
    return;
  }
...
int CLIO_CTE_DECL(fseek)(FILE *stream, long offset, int whence) {
  ...
  if (fd >= 0) {
    ...
    return (CLIO_CFS_CLIENT->SeekFd(fd, offset, whence) >= 0) ? 0 : -1;
  }
...
int CLIO_CTE_DECL(fseeko64)(FILE *stream, off64_t offset, int whence) {
  ...
  if (fd >= 0) {
    ...
    return (CLIO_CFS_CLIENT->SeekFd(fd, static_cast<off_t>(offset), whence) >= 0)
               ? 0
               : -1;
  }
...
int CLIO_CTE_DECL(fsetpos)(FILE *stream, const fpos_t *pos) {
  ...
  if (fd >= 0 && pos) {
    ...
    return (CLIO_CFS_CLIENT->SeekFd(fd, pos->__pos, SEEK_SET) >= 0) ? 0 : -1;
  }
...
long int CLIO_CTE_DECL(ftell)(FILE *fp) {
  ...
  if (fd >= 0) {
    ...
    return static_cast<long int>(CLIO_CFS_CLIENT->TellFd(fd));
  }
