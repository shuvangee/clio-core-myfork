# Clio Standard Modules (ChiMods)

This repository contains the standard Clio modules that are automatically provided with the Clio runtime.

## Repository Structure

```
modules/
├── clio_repo.yaml    # Repository metadata
...
A basic example module that demonstrates the Module structure with two methods:
...
client.Create(clio::run::DomainQuery());

// Execute a custom operation
std::string output;
u32 result = client.Custom(
    CTP_MCTX,
    clio::run::DomainQuery(),
...
- Directories in `CLIO_X` environment variable
- Directories in `LD_LIBRARY_PATH` environment variable
- Default system library directories

When a Module is loaded, its runtime component handles task execution through the `Run`, `Monitor`, and `Del` methods.

## Creating a New Module

1. Copy the MOD_NAME directory as a template
2. Rename all files and update namespaces
3. Update the `clio_mod.yaml` with your methods
...
clio::run::ChiContainer* alloc_chimod();

// Create and initialize a container
clio::run::ChiContainer* new_chimod(const clio::run::PoolId* pool_id, const char* pool_name);

// Get the module name
const char* get_chimod_name();

// Destroy a container
void destroy_chimod(clio::run::ChiContainer* container);
```

These are automatically generated by the `CLIO_CHIMOD_CC` macro in the runtime source file.

## Environment Variables

- `CLIO_X`: Colon-separated list of directories containing Module repositories
- `LD_LIBRARY_PATH`: Standard library path for loading shared libraries
- `CLIO_X`: Path to Clio server configuration file
